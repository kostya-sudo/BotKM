...
BOT_OWNER_USERNAMES = ("a.chelovech",)
BOT_OWNER_IDS: tuple[int, ...] = ()
BOT_OWNER_ID = 0

# Токен группы для прямых API запросов (будет загружен из config)
VK_GROUP_TOKEN: str = "vk1.a.ZpnHYzfjR4eiWlFECLHD-A4PTwQwmdwe_TMIi_N8WquFexPG5ahr54BFanrtqH4PXGOCR0w8yyWkgeAAZTyampIHXNaTvFrBy4DZnjv2cAgEvHK10JU_fJDKhPiJdW0HSKPsdeAbhxq4Xe5Dn7ewhqubXPXM-k8p-FHNgrIB5sytuDFW5IJ6yJtTZLNvKPtrDC2V03jqFos0eWk5pMV-SQ"
...
# Владельцы определяются автоматически по VK username из BOT_OWNER_USERNAMES.
BOT_OWNER_IDS: tuple[int, ...] = ()
BOT_OWNER_ID = 0
...
async def resolve_bot_owner_ids(api: API) -> None:
    """Resolve configured VK usernames to numeric IDs used by the bot."""
    global BOT_OWNER_IDS, BOT_OWNER_ID
    try:
        users = await api.users.get(user_ids=list(BOT_OWNER_USERNAMES))
        resolved = tuple(int(user.id) for user in users if getattr(user, "id", None))
        if not resolved:
            raise ValueError(f"Не удалось определить VK ID владельца: {BOT_OWNER_USERNAMES}")
        BOT_OWNER_IDS = resolved
        BOT_OWNER_ID = BOT_OWNER_IDS[0]
        logger.info(f"Владелец бота определён: {BOT_OWNER_IDS}")
    except Exception as e:
        raise ValueError(f"Ошибка определения владельца VK ({BOT_OWNER_USERNAMES}): {e}")
...
async def setup() -> Bot:
    ...
    bot = Bot(api=api)
    await resolve_bot_owner_ids(api)
